...
@Properties({
	    @Property(name =Constants.SERVICE_DESCRIPTION, value = "Activation failed payload path."),
	    @Property(name = Constants.SERVICE_VENDOR, value = "Adobe"),
	    @Property(name = "process.label", value = "Activate page workflow failed")
		
		})
	

public class MyPageNotActivatedWorkflow implements WorkflowProcess{



	Logger log = LoggerFactory.getLogger(MyPageNotActivatedWorkflow.class);
	...
	public void execute(WorkItem wItem, WorkflowSession wSession, MetaDataMap mDMap) throws WorkflowException {
		try {
			...
			if(session.nodeExists(payLoadPath+"/jcr:content")) {
				...
				n.setProperty("status","NotActivatetd");
				session.save();
				
			}

			log.debug("The {} page has not been acivated",payLoadPath);
